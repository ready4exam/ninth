...
export function renderQuestion(q, idx, selected, submitted) {
  ...
  els.list.innerHTML = `
    <div class="space-y-6">
      ...
      ${isARorCase && reason && !submitted ? `<p class="italic text-gray-700 border-l-4 border-blue-400 pl-4">Reason (R): ${reason}</p>` : ''}
      <div class="space-y-3">${optionsHtml}</div>
      ${submitted && isARorCase && reason ? `<div class="mt-3 p-3 bg-blue-50 rounded border-l-4 border-blue-500"><b>Explanation:</b> ${reason}</div>` : ''}
    </div>`;
    ...
    <div class="flex justify-center gap-3 flex-wrap">
      ${diffs.map(d => `
        <button data-diff="${d}" ${d===currentDifficulty?'disabled':''}
          class="px-5 py-2 rounded ${d===currentDifficulty?'bg-gray-300 text-gray-500':'bg-blue-600 text-white hover:bg-blue-700'}">
          ${labels[d]}
        </button>`).join('')}
    </div>
